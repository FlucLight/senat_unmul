...
<div class="flex items-center justify-end gap-1">
    ...
    @if ($document->isDraft())
        ...
        <form method="POST" action="{{ route('dokumen.destroy', $document) }}" onsubmit="return confirm('Hapus dokumen draft ini?')">
            ...
            <button type="submit" title="Hapus"
                class="inline-flex h-9 w-9 items-center justify-center rounded-md text-ink-400 transition hover:bg-error-bg hover:text-error-text">
                <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" aria-hidden="true">
    ...
    @else
        <a href="{{ route('dokumen.pdf', $document) }}" title="Download PDF"
            class="icon-action hover:text-green-700">
            <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" aria-hidden="true">
        ...
        @if ($document->qr_verification_url)
            <button type="button" title="Salin link verifikasi QR"
                ...
                class="copy-qr-link icon-action hover:text-green-700">
                <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" aria-hidden="true">
